...
actor MistiaStartupMaintenanceWorker {
    ...
    func cleanupOrphanedBillTransactions() throws -> [MistiaBootstrapDeleteMutation] {
        let context = ModelContext(modelContainer)
        return try MistiaBootstrap.cleanupOrphanedBillTransactions(modelContext: context)
    }
}
...
nonisolated enum MistiaBootstrap {
    ...
    @discardableResult
    static func cleanupOrphanedBillTransactions(
        modelContext: ModelContext
    ) throws -> [MistiaBootstrapDeleteMutation] {
        let allOccurrences = try modelContext.fetch(FetchDescriptor<DueOccurrenceRecord>())
        let allTransactions = try modelContext.fetch(FetchDescriptor<LedgerTransaction>())
        let allBills = try modelContext.fetch(FetchDescriptor<RecurringBillPlan>())
        let allInstallments = try modelContext.fetch(FetchDescriptor<InstallmentPlan>())

        let activeTransactionsByID = Dictionary(
            allTransactions.filter { $0.deletedAt == nil }.map { ($0.id, $0) },
            uniquingKeysWith: { first, _ in first }
        )
        let deletedBillIDs = Set(allBills.filter { $0.deletedAt != nil }.map(\.id))
        let deletedInstallmentIDs = Set(allInstallments.filter { $0.deletedAt != nil }.map(\.id))
        ...
        // 1. Pass 1: Occurrences linking to active transactions
        for occurrence in allOccurrences {
            guard let txID = occurrence.linkedTransactionID,
                  let transaction = activeTransactionsByID[txID] else {
                continue
            }

            let isOccurrenceDeleted = occurrence.deletedAt != nil
            let isNotPaid = occurrence.status != .paid
            let isParentPlanDeleted = (occurrence.sourceKind == .recurringBill && deletedBillIDs.contains(occurrence.sourceID))
                || (occurrence.sourceKind == .installment && deletedInstallmentIDs.contains(occurrence.sourceID))

            if isOccurrenceDeleted || isNotPaid || isParentPlanDeleted {
                transaction.markDeleted(at: now)
                deletedTransactionIDs.insert(transaction.id)
                deleteMutations.append(
                    MistiaBootstrapDeleteMutation(
                        entity: .transaction,
                        recordID: transaction.id,
                        modifiedAt: transaction.updatedAt
                    )
                ...
                if isNotPaid && !isOccurrenceDeleted {
                    occurrence.linkedTransactionID = nil
                    occurrence.updatedAt = now
                }
                if isParentPlanDeleted && !isOccurrenceDeleted {
        ...
        // 2. Pass 2: Orphaned transactions where occurrence was hard-deleted
        let validPaidTxIDs = Set(
            allOccurrences.filter {
                $0.deletedAt == nil && $0.status == .paid
            }.compactMap(\.linkedTransactionID)
        )

        var paidMonthKeysByBillID: [UUID: Set<String>] = [:]
        for occ in allOccurrences where occ.deletedAt == nil && occ.status == .paid && occ.sourceKind == .recurringBill {
            paidMonthKeysByBillID[occ.sourceID, default: []].insert(occ.selectedMonthKey)
        }

        var paidMonthKeysByInstallmentID: [UUID: Set<String>] = [:]
        for occ in allOccurrences where occ.deletedAt == nil && occ.status == .paid && occ.sourceKind == .installment {
            paidMonthKeysByInstallmentID[occ.sourceID, default: []].insert(occ.selectedMonthKey)
        }

        let calendar = MistiaCalendar.current
        let activeBillsByName = Dictionary(
            allBills.filter { $0.deletedAt == nil }.map { ($0.name.trimmingCharacters(in: .whitespacesAndNewlines).lowercased(), $0) },
            uniquingKeysWith: { first, _ in first }
        )
        let deletedBillsByName = Dictionary(
            allBills.filter { $0.deletedAt != nil }.map { ($0.name.trimmingCharacters(in: .whitespacesAndNewlines).lowercased(), $0) },
            uniquingKeysWith: { first, _ in first }
        )

        let activeInstallmentsByName = Dictionary(
            allInstallments.filter { $0.deletedAt == nil }.map { ($0.name.trimmingCharacters(in: .whitespacesAndNewlines).lowercased(), $0) },
            uniquingKeysWith: { first, _ in first }
        )
        let deletedInstallmentsByName = Dictionary(
            allInstallments.filter { $0.deletedAt != nil }.map { ($0.name.trimmingCharacters(in: .whitespacesAndNewlines).lowercased(), $0) },
            uniquingKeysWith: { first, _ in first }
        )

        for transaction in allTransactions where transaction.deletedAt == nil && !deletedTransactionIDs.contains(transaction.id) && !validPaidTxIDs.contains(transaction.id) {
            guard transaction.primaryKind == .expense, transaction.destinationWallet == nil else {
                continue
            }
            let titleNormalized = transaction.title.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            let txMonthKey = PlanningLogic.monthKey(for: transaction.occurredAt, calendar: calendar)

            var shouldDelete = false

            if let matchingActiveBill = activeBillsByName[titleNormalized] {
                let paidMonths = paidMonthKeysByBillID[matchingActiveBill.id] ?? []
                if !paidMonths.contains(txMonthKey) {
                    let isBillingCat = transaction.category?.systemKey == MistiaSystemCategoryKey.billing.rawValue
                        || (matchingActiveBill.category != nil && transaction.category?.id == matchingActiveBill.category?.id)
                        || (transaction.category?.systemKey != nil && transaction.category?.systemKey == matchingActiveBill.category?.systemKey)
                        || matchingActiveBill.category == nil
                    if isBillingCat {
                        shouldDelete = true
                    }
                }
            } else if let matchingDeletedBill = deletedBillsByName[titleNormalized] {
                let isBillingCat = transaction.category?.systemKey == MistiaSystemCategoryKey.billing.rawValue
                    || (matchingDeletedBill.category != nil && transaction.category?.id == matchingDeletedBill.category?.id)
                    || (transaction.category?.systemKey != nil && transaction.category?.systemKey == matchingDeletedBill.category?.systemKey)
                    || matchingDeletedBill.category == nil
                if isBillingCat {
                    shouldDelete = true
                }
            } else if let matchingActiveInstallment = activeInstallmentsByName[titleNormalized] {
                let paidMonths = paidMonthKeysByInstallmentID[matchingActiveInstallment.id] ?? []
                if !paidMonths.contains(txMonthKey) {
                    let isLoanCat = transaction.category?.systemKey == MistiaSystemCategoryKey.loanRepayment.rawValue
                    if isLoanCat {
                        shouldDelete = true
                    }
                }
            } else if let _ = deletedInstallmentsByName[titleNormalized] {
                let isLoanCat = transaction.category?.systemKey == MistiaSystemCategoryKey.loanRepayment.rawValue
                if isLoanCat {
                    shouldDelete = true
                }
    ...
    static func cleanupOrphanedBillTransactions(
        ...
        let deleteMutations = try cleanupOrphanedBillTransactions(modelContext: modelContext)
        for mutation in deleteMutations {
            sessionStore.recordDelete(
                entity: mutation.entity,
                recordID: mutation.recordID,
