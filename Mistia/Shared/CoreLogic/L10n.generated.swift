...
nonisolated enum L10n {
    ...
    nonisolated enum family {
        ...
        nonisolated enum family {
            ...
            static var upcoming: String { L10n.tr("family.family.upcoming", vi: "Sắp tới", en: "Upcoming", ja: "間もなく期限") }
            static func upcoming(language: MistiaAppLanguage) -> String { L10n.tr("family.family.upcoming", vi: "Sắp tới", en: "Upcoming", ja: "間もなく期限", language: language) }
    ...
    nonisolated enum management {
        ...
        nonisolated enum managementarchiveditems {
            ...
            static var debtTransaction: String { L10n.tr("management.managementarchiveditems.debtTransaction", vi: "Khoản công nợ", en: "Debt item", ja: "貸し借り取引") }
            static func debtTransaction(language: MistiaAppLanguage) -> String { L10n.tr("management.managementarchiveditems.debtTransaction", vi: "Khoản công nợ", en: "Debt item", ja: "貸し借り取引", language: language) }
        ...
        nonisolated enum managementauth {
            ...
            static var keepBothSidesMergeByRecordID: String { L10n.tr("management.managementauth.keepBothSidesMergeByRecordID", vi: "Giữ cả hai phía, gộp theo ID, không tự động nhập nhằng thu chi giống nhau.", en: "Keep both sides, merge by record ID, and avoid risky automatic cashflow item dedupe.", ja: "両側のデータを保持し、レコード ID で統合しつつ危険な自動重複排除は行いません。") }
            static func keepBothSidesMergeByRecordID(language: MistiaAppLanguage) -> String { L10n.tr("management.managementauth.keepBothSidesMergeByRecordID", vi: "Giữ cả hai phía, gộp theo ID, không tự động nhập nhằng thu chi giống nhau.", en: "Keep both sides, merge by record ID, and avoid risky automatic cashflow item dedupe.", ja: "両側のデータを保持し、レコード ID で統合しつつ危険な自動重複排除は行いません。", language: language) }
            ...
            static func mistiaKeptBothRecordsSafelyThereAre(_ value: String) -> String {
                L10n.format("management.managementauth.mistiaKeptBothRecordsSafelyThereAre", vi: "Mistia đang giữ an toàn cả hai bản ghi. Hiện có %@ thu chi cần bạn rà lại sau sync.", en: "Mistia kept both records safely. There are currently %@ cashflow items to review after sync.", ja: "両方のレコードを安全に保持しています。同期後に確認が必要な取引が %@ 件あります。", value)
            }
            static func mistiaKeptBothRecordsSafelyThereAre(_ value: String, language: MistiaAppLanguage) -> String {
                L10n.format("management.managementauth.mistiaKeptBothRecordsSafelyThereAre", vi: "Mistia đang giữ an toàn cả hai bản ghi. Hiện có %@ thu chi cần bạn rà lại sau sync.", en: "Mistia kept both records safely. There are currently %@ cashflow items to review after sync.", ja: "両方のレコードを安全に保持しています。同期後に確認が必要な取引が %@ 件あります。", language: language, value)
            }
            ...
            static var possibleDuplicatesDetected: String { L10n.tr("management.managementauth.possibleDuplicatesDetected", vi: "Có thu chi có thể bị trùng", en: "Possible duplicates detected", ja: "重複の可能性がある取引があります") }
            static func possibleDuplicatesDetected(language: MistiaAppLanguage) -> String { L10n.tr("management.managementauth.possibleDuplicatesDetected", vi: "Có thu chi có thể bị trùng", en: "Possible duplicates detected", ja: "重複の可能性がある取引があります", language: language) }
    ...
    nonisolated enum planning {
        ...
        nonisolated enum planning {
            ...
            static var archivedBudgetsWillNoLongerAppearIn: String { L10n.tr("planning.planning.archivedBudgetsWillNoLongerAppearIn", vi: "Ngân sách lưu trữ sẽ không còn hiện trong tab Sắp tới.", en: "Archived budgets will no longer appear in Upcoming.", ja: "アーカイブした予算はプラン画面に表示されなくなります。") }
            static func archivedBudgetsWillNoLongerAppearIn(language: MistiaAppLanguage) -> String { L10n.tr("planning.planning.archivedBudgetsWillNoLongerAppearIn", vi: "Ngân sách lưu trữ sẽ không còn hiện trong tab Sắp tới.", en: "Archived budgets will no longer appear in Upcoming.", ja: "アーカイブした予算はプラン画面に表示されなくなります。", language: language) }
        ...
        nonisolated enum session {
            ...
            nonisolated enum session {
                ...
                static func mistiaFoundValuePossibleDuplicateTransactionsAnd(_ value: String) -> String {
                    L10n.format("shared.session.session.mistiaFoundValuePossibleDuplicateTransactionsAnd", vi: "Mistia thấy %@ thu chi có thể bị trùng và đang giữ an toàn cả hai bản ghi.", en: "Mistia found %@ possible duplicate cashflow items and kept both records safely.", ja: "重複の可能性がある取引を %@ 件検出したため, 両方のレコードを安全に保持しています。", value)
                }
                static func mistiaFoundValuePossibleDuplicateTransactionsAnd(_ value: String, language: MistiaAppLanguage) -> String {
                    L10n.format("shared.session.session.mistiaFoundValuePossibleDuplicateTransactionsAnd", vi: "Mistia thấy %@ thu chi có thể bị trùng và đang giữ an toàn cả hai bản ghi.", en: "Mistia found %@ possible duplicate cashflow items and kept both records safely.", ja: "重複の可能性がある取引を %@ 件検出したため, 両方のレコードを安全に保持しています。", language: language, value)
                }
    ...
    nonisolated enum transactions {
        ...
        nonisolated enum aibill {
            ...
            static var createTransaction: String { L10n.tr("transactions.aibill.createTransaction", vi: "Tạo khoản thu chi", en: "Create cashflow item", ja: "収支を作成") }
            static func createTransaction(language: MistiaAppLanguage) -> String { L10n.tr("transactions.aibill.createTransaction", vi: "Tạo khoản thu chi", en: "Create cashflow item", ja: "収支を作成", language: language) }
        ...
        nonisolated enum transactioneditor {
            ...
            static var chooseTheWalletUsedForThisDebt: String { L10n.tr("transactions.transactioneditor.chooseTheWalletUsedForThisDebt", vi: "Chọn ví thực hiện khoản công nợ.", en: "Choose the wallet used for this debt item.", ja: "この貸し借り取引で使うウォレットを選択してください。") }
            static func chooseTheWalletUsedForThisDebt(language: MistiaAppLanguage) -> String { L10n.tr("transactions.transactioneditor.chooseTheWalletUsedForThisDebt", vi: "Chọn ví thực hiện khoản công nợ.", en: "Choose the wallet used for this debt item.", ja: "この貸し借り取引で使うウォレットを選択してください。", language: language) }
            ...
            static var familyTransferReadOnlyNotice: String { L10n.tr("transactions.transactioneditor.familyTransferReadOnlyNotice", vi: "Khoản chuyển tiền gia đình chỉ dùng để xem chi tiết và không thể sửa, lưu trữ hoặc xóa.", en: "Family transfer records are detail-only and cannot be edited, archived, or deleted.", ja: "家族送金の記録は詳細表示のみで、編集、アーカイブ、削除はできません。") }
            static func familyTransferReadOnlyNotice(language: MistiaAppLanguage) -> String { L10n.tr("transactions.transactioneditor.familyTransferReadOnlyNotice", vi: "Khoản chuyển tiền gia đình chỉ dùng để xem chi tiết và không thể sửa, lưu trữ hoặc xóa.", en: "Family transfer records are detail-only and cannot be edited, archived, or deleted.", ja: "家族送金の記録は詳細表示のみで、編集、アーカイブ、削除はできません。", language: language) }
            ...
            static var thisTransactionIsPartOfAPaid: String { L10n.tr("transactions.transactioneditor.thisTransactionIsPartOfAPaid", vi: "Thu chi này thuộc sao kê đã thanh toán nên không thể sửa đổi hoặc lưu trữ.", en: "This cashflow item is part of a paid statement and cannot be modified or archived.", ja: "この取引は支払い済みの明細に含まれているため、変更やアーカイブはできません。") }
            static func thisTransactionIsPartOfAPaid(language: MistiaAppLanguage) -> String { L10n.tr("transactions.transactioneditor.thisTransactionIsPartOfAPaid", vi: "Thu chi này thuộc sao kê đã thanh toán nên không thể sửa đổi hoặc lưu trữ.", en: "This cashflow item is part of a paid statement and cannot be modified or archived.", ja: "この取引は支払い済みの明細に含まれているため、変更やアーカイブはできません。", language: language) }
