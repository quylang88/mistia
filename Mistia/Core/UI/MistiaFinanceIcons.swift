...
nonisolated enum MistiaFinanceIconRegistry {
    static func descriptor(for token: String) -> MistiaFinanceIconDescriptor? {
        if let category = MistiaSystemCategoryRegistry.shared.category(forIcon: token) {
            return descriptor(
                token: token,
    ...
    private static let explicitDescriptors: [String: MistiaFinanceIconDescriptor] = [
        ...
        "mistia.settlement.event": descriptor(token: "mistia.settlement.event", fallbackSystemName: "person.2.fill", baseHex: "#A76BFF", group: .planning, badgeSystemName: "calendar.badge.clock"),
