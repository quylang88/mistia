-- Let the app pass localized titles for new family-transfer transaction rows.
-- Existing hardcoded titles are left untouched.

drop function if exists public.create_family_transfer(
...
create or replace function public.create_family_transfer(
    ...
    p_sender_title text default null,
    p_recipient_title text default null
)
returns jsonb
language plpgsql
security definer
...
declare
    ...
    sender_note text;
    recipient_note text;
    sender_title text := coalesce(nullif(trim(p_sender_title), ''), 'Chuyển tiền gia đình');
    recipient_title text := coalesce(nullif(trim(p_recipient_title), ''), 'Nhận tiền gia đình');
...
begin
    ...
    values (
        ...
        'posted', sender_title, sender_note, p_amount_minor,
    ...
    values (
        ...
        'posted', recipient_title, recipient_note, resolved_destination_amount_minor,
