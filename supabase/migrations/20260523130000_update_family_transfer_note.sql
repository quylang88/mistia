-- Update create_family_transfer to support optional notes
...
begin
    ...
    if p_note is not null and trim(p_note) <> '' then
        sender_note := sender_note || chr(10) || chr(10) || trim(p_note);
        recipient_note := recipient_note || chr(10) || chr(10) || trim(p_note);
    end if;
...
grant execute on function public.create_family_transfer(uuid, uuid, uuid, uuid, bigint, timestamptz, text) to authenticated;
