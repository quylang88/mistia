-- Allow explicit zero-amount Investment trades:
-- - sell: total-loss write-off / liquidation at 0;
-- - buy: promotional or gifted inventory acquired at 0.
...
-- 3. Update mutate_investment_trade to validate the kind-specific zero-amount wallet shape.
...
alter table public.investment_trades
    ...
    add constraint investment_trades_wallet_shape_check check (
        ...
        or (kind_raw_value = 'buy' and gross_amount_minor = 0 and accounting_gross_amount_minor = 0 and funding_wallet_id is null and capital_return_wallet_id is null)
...
begin
    ...
    if incoming.kind_raw_value not in ('buy', 'sell')
       ...
       or not (
           (incoming.gross_amount_minor = 0 and incoming.accounting_gross_amount_minor = 0)
           or (incoming.gross_amount_minor > 0 and incoming.accounting_gross_amount_minor > 0)
       )
    ...
    if incoming.kind_raw_value = 'buy' then
        if incoming.gross_amount_minor > 0 then
            if incoming.funding_wallet_id is null or incoming.capital_return_wallet_id is not null then
                raise exception 'A positive-amount buy requires one funding wallet';
            end if;
            select * into wallet_row from public.ledger_wallets
            ...
            if incoming.funding_wallet_id is not null or incoming.capital_return_wallet_id is not null then
                raise exception 'A zero-amount promotional buy must not have a wallet';
            end if;
        end if;
    else
