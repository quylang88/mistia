-- Cloud wallet current_balance_minor is a display snapshot. For credit cards,
-- store available credit, not internal debt.
...
create or replace function public.mistia_refresh_wallet_balance_from_credit_card_profile()
returns trigger
language plpgsql
security definer
...
begin
  if tg_op = 'INSERT' then
    if new.wallet_id is not null then
      perform public.mistia_recalculate_wallet_current_balance(new.wallet_id);
    end if;
  elsif tg_op = 'UPDATE' then
    if old.wallet_id is not null then
      perform public.mistia_recalculate_wallet_current_balance(old.wallet_id);
    end if;

    if new.wallet_id is not null and new.wallet_id is distinct from old.wallet_id then
      perform public.mistia_recalculate_wallet_current_balance(new.wallet_id);
    end if;
...
drop trigger if exists credit_card_profiles_refresh_wallet_balances on public.credit_card_profiles;
create trigger credit_card_profiles_refresh_wallet_balances
after insert or update or delete on public.credit_card_profiles
for each row
execute function public.mistia_refresh_wallet_balance_from_credit_card_profile();
