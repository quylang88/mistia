-- Keep the cloud wallet snapshot aligned with TransactionLogic.walletBalanceIndex.
-- Family transfers encode both directions on source_wallet_id: a null
-- destination means money received, while a non-null destination means money sent.
    ...
    select
      ...
      + coalesce(
        (
          select sum(
            case
              when tx.source_wallet_id = p_wallet_id
                and tx.primary_kind_raw_value = 'transfer'
                and tx.transfer_subtype_raw_value = 'familyTransfer' then
                public.mistia_wallet_balance_delta(
                  p_wallet_kind,
                  case when tx.destination_wallet_id is null then 'income' else 'expense' end,
                  null,
                  null,
                  tx.amount_minor,
                  'source'
            ...
            +
            case
              when tx.destination_wallet_id = p_wallet_id
                and tx.primary_kind_raw_value = 'transfer'
                and tx.transfer_subtype_raw_value = 'familyTransfer' then 0
